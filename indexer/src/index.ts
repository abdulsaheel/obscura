...
import { IncrementalMerkleTree, TREE_LEVELS } from './merkle.js';
...
interface DatabaseWithStatements extends sqlite3.Database {
  statements: {
    ...
    confirmAllVaultConfirmations: sqlite3.Statement;
...
const CONFIG = {
  ...
  // P2P announcement path is EXPERIMENTAL and disabled by default. The trusted
  // source of truth is the on-chain polling path (pollForVaultEvents). Only
  // enable this once announcement signatures are cryptographically verified.
  enableP2P: process.env.ENABLE_P2P === 'true',
  // Block to start scanning Deposit events from when reconstructing a vault's
  // Merkle tree. Use the vault factory deployment block in production to avoid
  // scanning the entire chain on every cold cache.
  startBlock: process.env.START_BLOCK ? parseInt(process.env.START_BLOCK) : 0,
  // Max block span per getLogs call (most RPCs cap range at 10k-50k blocks).
  logChunkSize: process.env.LOG_CHUNK_SIZE ? parseInt(process.env.LOG_CHUNK_SIZE) : 50000,
  // TTL (ms) for the per-vault reconstructed-leaves cache.
  merkleCacheTtlMs: process.env.MERKLE_CACHE_TTL_MS ? parseInt(process.env.MERKLE_CACHE_TTL_MS) : 30000,
    ...
    const dbInstance = new sqlite3.Database(CONFIG.dbPath, (err) => {
      ...
      dbInstance.exec(createTablesSQL, (err) => {
        ...
        (dbInstance as DatabaseWithStatements).statements = {
          ...
          getPeerById: dbInstance.prepare(`
          ...
          confirmAllVaultConfirmations: dbInstance.prepare(`
            UPDATE vault_confirmations SET confirmed = TRUE
            WHERE vault_address = ?
          `),
...
// P2P Node setup
// EXPERIMENTAL: the P2P announcement path is disabled unless CONFIG.enableP2P
// (env ENABLE_P2P=true). Announcement signatures are not yet cryptographically
// verified, so a malicious peer could otherwise inject vaults. The on-chain
// poller (pollForVaultEvents) is the trusted, primary discovery mechanism.
async function createP2PNode() {
  if (!CONFIG.enableP2P) {
    logger.warn('P2P node disabled (set ENABLE_P2P=true to enable the experimental P2P path)');
    return undefined;
  }
  try {
...
async function handleVaultAnnouncement(message: any, fromPeer: string) {
  // EXPERIMENTAL P2P path: refuse to process announcements unless explicitly
  // enabled. The on-chain poller is the trusted source of truth.
  if (!CONFIG.enableP2P) {
    logger.debug('Ignoring vault announcement: P2P path disabled', { fromPeer });
    return;
  }
...
async function checkPeerReputation(peerId: string): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    // BUGFIX: previously queried the `vaults` table (getVaultByAddress) with a
    // peerId, which never matched. Query the `peers` table by peer_id instead.
    db.statements.getPeerById.get(peerId, (err, row: any) => {
      if (err || !row) {
        resolve(true); // New peer (no row), assume reputable
        return;
      }
...
// Per-peer announcement timestamps (ms) within a sliding 1-hour window.
// In-memory is acceptable: it only protects the experimental P2P path, and the
// limiter fails closed within a single process lifetime.
const announcementWindowMs = 60 * 60 * 1000;
const announcementTimestamps = new Map<string, number[]>();

/**
 * Sliding-window rate limiter for P2P announcements. Rejects a peer if it has
 * already made more than CONFIG.maxUnconfirmedAnnouncements announcements within
 * the last hour. Records the current announcement when accepted.
 */
async function checkAnnouncementRateLimit(peerId: string): Promise<boolean> {
  const now = Date.now();
  const cutoff = now - announcementWindowMs;

  const recent = (announcementTimestamps.get(peerId) || []).filter((t) => t > cutoff);

  if (recent.length >= CONFIG.maxUnconfirmedAnnouncements) {
    // Persist the pruned window so it does not grow unbounded, but do not
    // record this rejected attempt.
    announcementTimestamps.set(peerId, recent);
    return false;
  }
...
async function verifyAnnouncementSignature(message: any, signature: string, peerId: string): Promise<boolean> {
  // NOTE: There is no real cryptographic verification of P2P announcement
  // signatures yet (it requires the peer's public key + a defined signing
  // scheme). We deliberately do NOT silently return true, which would let any
  // peer forge announcements. Until a real scheme exists, treat every signed
  // announcement as unverifiable (false). The entire P2P path is additionally
  // gated behind CONFIG.enableP2P and the on-chain poller is the trusted
  // source of truth.
  logger.warn('Announcement signature verification not implemented; rejecting', { peerId });
  return false;
}
...
async function confirmAllVaultConfirmations(vaultAddress: string): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    db.statements.confirmAllVaultConfirmations.run(
      vaultAddress,
      function (err: Error | null) {
        if (err) reject(err);
        else resolve();
...
//
// To serve a Merkle proof for a vault we must rebuild the exact leaf ordering
// the contract used. We do that by reading every Deposit event from the vault,
// ordering by the on-chain leafIndex, and feeding the commitments into the
// canonical incremental Merkle tree (see src/merkle.ts). The reconstructed
// leaves are cached per-vault with a short TTL so repeated requests don't
// re-scan the whole log range.
// ---------------------------------------------------------------------------
...
// De-duplicate concurrent rebuilds for the same vault so we don't fire N log
// scans when N requests arrive simultaneously on a cold cache.
const inflightLeafFetches = new Map<string, Promise<bigint[]>>();

/**
 * Fetch and order all Deposit commitments for a vault via chunked getLogs.
 * Returns the leaves in ascending leafIndex order (the order the contract
 * inserted them into its incremental tree).
 */
async function fetchVaultLeaves(vaultAddress: string): Promise<bigint[]> {
  if (!provider) {
    throw new Error('Ethereum provider not initialized');
  }

  const vault = new ethers.Contract(vaultAddress, VAULT_DEPOSIT_ABI, provider);
  const depositFilter = vault.filters.Deposit();

  const latestBlock = await provider.getBlockNumber();
  const chunk = Math.max(1, CONFIG.logChunkSize);
  const fromBlockStart = Math.max(0, CONFIG.startBlock);

  // Collect (leafIndex, commitment) pairs so we can sort deterministically.
  const collected: Array<{ leafIndex: number; commitment: bigint }> = [];

  for (let from = fromBlockStart; from <= latestBlock; from += chunk) {
    const to = Math.min(from + chunk - 1, latestBlock);
    const events = await vault.queryFilter(depositFilter, from, to);
    for (const event of events) {
      if (!('args' in event) || !event.args) continue;
      const commitment = BigInt(event.args[0]);
      const leafIndex = Number(event.args[1]);
      collected.push({ leafIndex, commitment });
    }
  }

  collected.sort((a, b) => a.leafIndex - b.leafIndex);

  const leaves = collected.map((c) => c.commitment);

  logger.debug('Reconstructed vault leaves', {
    vault: vaultAddress,
...
/**
 * Return the reconstructed leaves for a vault, using the short-TTL cache when
 * fresh and coalescing concurrent rebuilds.
 */
async function getVaultLeaves(vaultAddress: string): Promise<bigint[]> {
  const key = vaultAddress.toLowerCase();
  const now = Date.now();

  const cached = leafCache.get(key);
  if (cached && now - cached.fetchedAt < CONFIG.merkleCacheTtlMs) {
    return cached.leaves;
  }

  const existing = inflightLeafFetches.get(key);
  if (existing) return existing;

  const promise = (async () => {
    try {
      const leaves = await fetchVaultLeaves(vaultAddress);
      leafCache.set(key, { leaves, fetchedAt: Date.now() });
      return leaves;
    } finally {
      inflightLeafFetches.delete(key);
    }
  })();

  inflightLeafFetches.set(key, promise);
  return promise;
}
...
app.get('/vaults/:address/merkle-proof', async (req, res) => {
  const { address } = req.params;
  const leafIndexRaw = req.query.leafIndex;

  // Validate address shape early so we don't fire RPC for obvious garbage.
  if (!ethers.isAddress(address)) {
    return res.status(400).json({ error: 'Invalid vault address' });
  }

  // Validate leafIndex: must be present and a non-negative integer.
  if (leafIndexRaw === undefined || Array.isArray(leafIndexRaw)) {
    return res.status(400).json({ error: 'Missing or invalid leafIndex query parameter' });
  }
  const leafIndexStr = String(leafIndexRaw);
  if (!/^\d+$/.test(leafIndexStr)) {
  ...
  if (!Number.isSafeInteger(leafIndex) || leafIndex < 0) {
    return res.status(400).json({ error: 'leafIndex out of range' });
  }

  try {
    const leaves = await getVaultLeaves(address);

    if (leafIndex >= leaves.length) {
      return res.status(404).json({
        error: 'leafIndex exceeds number of deposits',
        leafIndex,
        depositCount: leaves.length
      });
    }
    ...
    logger.error('Failed to build merkle proof', {
      address,
      leafIndex,
      error: error instanceof Error ? error.message : String(error)
    });
...
// Scheduled tasks
// Registered from main() so that importing this module (e.g. from tests) does
// not spin up background cron timers and keep the event loop alive.
...
async function main() {
  try {
    ...
    // Register cron tasks now that the DB/provider exist.
    registerScheduledTasks();

    // EXPERIMENTAL: only attempt to bring up the P2P node when explicitly
    // enabled. The on-chain poller is the primary, trusted discovery path.
    if (CONFIG.enableP2P) {
      try {
        await createP2PNode();
      } catch (p2pError) {
...
// Start the indexer only when this module is executed directly (e.g.
// `node dist/index.js`). When imported (tests via supertest, the merkle proof
// helper, etc.) we skip startup so no server is opened and no DB/timers run.
const isMainModule = (() => {
...
export { app, db, node, CONFIG, main, initializeDatabase, fetchVaultLeaves, getVaultLeaves };