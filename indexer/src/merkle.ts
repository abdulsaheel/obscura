 ...
 *
 * This is a faithful, typed port of scripts/lib/merkle.js. It MUST reproduce the
 * exact root/proof semantics of PrivateVault.sol:
 *   - Poseidon(t=3) two-input hash (circomlib-compatible, via poseidon-lite)
 *   - zero chain: zeros[0] = 0n, zeros[i+1] = poseidon2(zeros[i], zeros[i])
 ...
 * contract's incremental insertion, so this is used by the indexer's
 * /merkle-proof endpoint and as a cross-checked reference for the frontend.
 */
...
/**
 * Build the zero-value chain for a tree of `levels` levels.
 * zeros[0] = 0n, zeros[i+1] = poseidon2(zeros[i], zeros[i]).
 */
...
export class IncrementalMerkleTree {
  ...
  root(): bigint {
    let level: bigint[] = this.leaves.length
      ? this.leaves.map((l) => BigInt(l))
      : [this.zeros[0]];
