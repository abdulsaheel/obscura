...
4. **Range Checks:**
   ```circom
   # Amount must be a positive value that fits in 128 bits
   amount != 0
   amount < 2^128

   # Fee must not exceed the withdrawn amount, and fits in 128 bits
   protocolFee < 2^128
   protocolFee <= amount

   # Recipient must be a valid 160-bit Ethereum address
   recipient < 2^160
   ```

   > **Why not enforce the 1% fee cap in-circuit?** Division inside an R1CS
   > circuit (`amount / 100`) is a *field inverse*, not integer division, so a
   > constraint like `protocolFee <= amount / 100` is mathematically meaningless
   > over the scalar field — it does not express "1% of the amount". The circuit
   > therefore only binds `protocolFee <= amount`; the exact **1% policy is
   > enforced on-chain** with real integer arithmetic in `PrivateVault.withdraw`
   > (`protocolFee <= amount * MAX_FEE_BASIS_POINTS / 10000`).
   >
   > The range checks use **128-bit** gadgets. The earlier 64-bit gadgets
   > overflowed: deposits are denominated in wei, and 100 ETH = 1e20 wei ≈ 2^66.5,
   > so any deposit above ~18.44 ETH (2^64 wei) triggered a `Num2Bits` assertion
   > failure. 128 bits covers the full 0.001–100 ETH range with large headroom.
...
**Circuit Statistics:**
- **Constraints:** ~12,077 total (5,975 non-linear + ~6,102 linear)
- **Public Inputs:** 5 — `[nullifierHash, root, recipient, protocolFee, amount]`
...
2. **Proof Generation Time:**
   ...
   - Current: ~12,077 constraints → 30-60s
...
Obscura's Groth16 proving/verification keys are derived from a two-phase
trusted setup. The whole pipeline is scripted as `npm run build-all` in
`circuits/`.

**Phase 1: Powers of Tau (reused from PSE)**

Rather than running a fresh, low-participation Powers of Tau, Obscura reuses the
PSE **Perpetual Powers of Tau** ceremony — specifically contribution **#0080** at
degree 14 (`ppot_0080_14.ptau`). This is a long-running multi-party ceremony with
thousands of independent contributors, so the phase-1 honesty assumption rests on
that broad set rather than a handful of self-selected participants.

```bash
# Download + verify the PSE Perpetual Powers of Tau contribution #0080 (deg 14)
curl -fL -o ppot_0080_14.ptau \
  https://pse-trusted-setup-ppot.s3.amazonaws.com/pot28_0080/ppot_0080_14.ptau
snarkjs powersoftau verify ppot_0080_14.ptau
```

**Phase 2: Circuit-Specific Setup**
```bash
# Link circuit R1CS to the phase-1 powers
snarkjs groth16 setup private-transfer.r1cs ppot_0080_14.ptau private-transfer_0000.zkey

# Obscura phase-2 contribution (fresh random entropy)
snarkjs zkey contribute private-transfer_0000.zkey private-transfer_0001.zkey \
  --name="Obscura phase2 contribution"

# Finalize with a public verifiable-delay beacon
snarkjs zkey beacon private-transfer_0001.zkey private-transfer_final.zkey <beacon> 10

# Export the Solidity verifier
snarkjs zkey export solidityverifier private-transfer_final.zkey ../contracts/Verifier.sol
```

**Security Assumption:**
At least one honest contributor in Phase 1 (satisfied by the PSE ceremony) AND
one honest contributor in Phase 2 (the Obscura contribution) must delete their
toxic waste. If this holds, the setup is sound. The final beacon removes the
last contributor's ability to bias the result.

**Verification:**
Anyone can verify the phase-2 key against the R1CS and the PSE ptau:
```bash
snarkjs zkey verify private-transfer.r1cs ppot_0080_14.ptau private-transfer_final.zkey
```
...
template PrivateTransfer(levels) {
    ...
    //     NOTE: the exact "fee <= 1%" policy is enforced ON-CHAIN with real
    //     integer division — in-circuit `/` is a field inverse, so a 1% check
